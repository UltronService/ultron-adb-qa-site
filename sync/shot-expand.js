...
(async () => {
  ...
  const expand = p.getByRole("button", { name: /展開完整欄位/ });
  await expand.click();
  await p.waitForTimeout(1500);
  const text = await p.locator("body").innerText();
  console.log("SNIP", JSON.stringify(text.slice(0, 1200)));
  await p.screenshot({ path: "C:/Users/User/Desktop/qa_schedule_expanded.png", fullPage: true });
  await b.close();
  console.log("DONE");
})().catch((e) => {
  console.error(e);
